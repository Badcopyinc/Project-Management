...
// Final working JS will be inserted here — this step ensures it matches all fixes
let isEditing = false;
...
function render() {
  ...
  savedData.projects.forEach((project, pIndex) => {
    ...
    project.tasks.forEach((task, tIndex) => {
      ...
      const taskTitle = isEditing ? document.createElement("input") : document.createElement("strong");
      if (isEditing) {
        taskTitle.type = "text";
        taskTitle.value = task.name;
        taskTitle.oninput = () => { savedData.projects[pIndex].tasks[tIndex].name = taskTitle.value; saveData(); };
      } else taskTitle.textContent = task.name;
      ...
      task.subtasks.forEach((sub, sIndex) => {
        ...
        if (isEditing) {
          ...
          checkbox.onchange = () => { savedData.progress[key] = checkbox.checked; saveData(); render(); };
    ...
    project.materials.forEach((mat, mIndex) => {
      ...
      const matTitle = isEditing ? document.createElement("input") : document.createElement("strong");
      if (isEditing) {
        matTitle.type = "text";
      ...
      } else matTitle.textContent = mat.name;
      ...
      mat.subtasks.forEach((sub, sIndex) => {
        ...
        const label = isEditing ? document.createElement("input") : document.createElement("label");
        if (isEditing) {
          label.type = "text";
          label.value = sub;
          label.oninput = () => {
        ...
        } else label.textContent = " " + sub;

        const stageLabel = document.createElement("span");
        if (!isEditing && stage > 0 && materialStages[stage]) {
        ...
        item.appendChild(checkbox);
        item.appendChild(label);
        if (!isEditing && stage > 0) item.appendChild(stageLabel);
